...
// Clock schedules cancellable tasks for execution.
type Clock interface {
	// AfterFunc waits for the duration to elapse, performs a lock if possible,
	// then calls f in its own goroutine. It returns a function that can be used
	// to cancel the call.
	AfterFunc(l sync.Locker, d time.Duration, f func()) context.CancelFunc
}

// systemClock schedules work using the system clock via CancellableTimer.
type systemClock struct{}

var _ Clock = (*systemClock)(nil)

func (*systemClock) AfterFunc(l sync.Locker, d time.Duration, f func()) context.CancelFunc {
...
// neighborEntry.
func newNeighborEntry(clock Clock, nic *NIC, remoteAddr tcpip.Address, localAddr tcpip.Address, nudState *NUDState, linkRes LinkAddressResolver) *neighborEntry {
	e := &neighborEntry{
		clock:    clock,
		nic:      nic,
		nudState: nudState,
...
// `setStateLocked`.
func newStaticNeighborEntry(clock Clock, nic *NIC, addr tcpip.Address, linkAddr tcpip.LinkAddress, state *NUDState) *neighborEntry {
...
func (e *neighborEntry) setStateLocked(next NeighborState) {
	...
	// Stale, or Static state do not have scheduled actions.
	if cancel := e.mu.cancel; cancel != nil {
		cancel()
	}
	...
	case Incomplete:
		...
		sendMulticastProbe = func() {
			...
			retryCounter++
			e.mu.cancel = e.clock.AfterFunc(&e.mu, config.RetransmitTimer, sendMulticastProbe)
		}

		sendMulticastProbe()

	case Reachable:
		e.mu.cancel = e.clock.AfterFunc(&e.mu, e.nudState.ReachableTime(), func() {
			e.dispatchChangeEventLocked(Stale)
			e.setStateLocked(Stale)
		})

	case Delay:
		e.mu.cancel = e.clock.AfterFunc(&e.mu, config.DelayFirstProbeTime, func() {
	...
	case Probe:
		...
		sendUnicastProbe = func() {
			...
			e.mu.cancel = e.clock.AfterFunc(&e.mu, config.RetransmitTimer, sendUnicastProbe)
		}
	...
	case Failed:
		...
		e.mu.cancel = e.clock.AfterFunc(&e.mu, config.UnreachableTime, func() {
