...
// dir and environment.
func ResolveExecutablePath(ctx context.Context, creds *auth.Credentials, mns *fs.MountNamespace, mnsVFS2 *vfs.MountNamespace, envv []string, wd, name string) (string, error) {
	if (mns == nil && mnsVFS2 == nil) || (mns != nil && mnsVFS2 != nil) {
		panic(fmt.Sprintf("exactly one of mns and mnsVFS2 must be passed. Got mns=%v mnsVFS2=%v", mns, mnsVFS2))
	}
