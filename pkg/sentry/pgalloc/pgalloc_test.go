	...
	for _, test := range []struct {
		...
		t.Run(test.desc, func(t *testing.T) {
			...
			if !test.expectFail && !ok {
				t.Fatalf("findAvailableRange(%v, %x, %x, %x): got %x, false wanted %x, true", test.usage, test.fileSize, test.length, test.alignment, fr.Start)
			}
			if test.expectFail && ok {
				t.Fatalf("findAvailableRange(%v, %x, %x, %x): got %x, true wanted %x, false", test.usage, test.fileSize, test.length, test.alignment, fr.Start)
			}
